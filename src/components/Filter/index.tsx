...
//
//     setFilterButtonIsDisabled((!selectedValuesHasValue && isFilterOperatorNotAnyOrNone) || isFilterReady)
...
//       </FormGroup>
//     )
//   }, [builtFilter?.operator, builtFilter.selectedValues, changeFilters, checkboxStates, currentFilterCheckboxes])
//
...
//           <Box sx={{ display: quickSearchIsOn ? 'none' : 'block' }}>
...
//       )}
//       <Fade in={quickSearchIsOn} timeout={700}>
//         <Box sx={{ display: quickSearchIsOn ? 'block' : 'none' }}>
...
const FilterButtonIcon = React.memo(({ builtFilter }: { builtFilter: IBuiltFilter }) => (
  builtFilter?.ready && builtFilter?.operator?.id !== 'any' && builtFilter?.operator?.id !== 'none'
    ? <NorthIcon /> : <FilterAltIcon />
))
